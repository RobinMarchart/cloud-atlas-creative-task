...
function print_flame(draw,base_x,base_y,min_x,min_y,max_x,max_y,point){
    ...
    var arc=point.arc*180/Math.PI;
    if((x>=min_x)&&(y>=min_y)&&(x<=max_x)&&(y<=max_y))draw.plain("\u{1F525}").move(x,y).rotate(arc,x,y);
}

function print_flame_w(draw,base_x,base_y,min_x,min_y,max_x,max_y,point){
    var x=point.x+base_x;
    var y=point.y+base_y;
...
function create() {
    console.info("Creating new graphic");
    var i = image_id++
    ...
    var flm=(document.getElementById("spiral-special").checked)?print_flame_w.bind(null,draw,250,250,-40,-40,540,540):print_flame.bind(null,draw,250,250,-40,-40,540,540);
    for (let rad of radii){
        for(let offset of offsets){
            flm(pnt(rad,offset));
        };
    };
    console.info("finished creating image")
